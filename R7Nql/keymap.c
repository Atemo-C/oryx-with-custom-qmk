...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
	...
	switch (keycode) {
		...
		case ST_MACRO_1:
		if (record->event.pressed) {
			SEND_STRING(SS_LCTL(SS_LSFT(SS_TAP(X_U))) SS_DELAY(2) SS_TAP(X_KP_2) SS_DELAY(2) SS_TAP(X_KP_0) SS_DELAY(2) SS_TAP(X_KP_2) SS_DELAY(2) SS_TAP(X_KP_6)	SS_DELAY(2) SS_TAP(X_SPACE) SS_DELAY(2) SS_TO(0));
		}
		break;
